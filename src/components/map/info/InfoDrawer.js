                    ...
                    <TableHead>
                        <TableRow>
                            {[`${txt.mapInfo.stats}\n${this.props.statisticYear}`, ...statistics].map((item, index) => {
                                if (index === 0) {
                                    return <TableCell style={{ fontWeight: 800, textTransform: 'uppercase' }} padding='none' key={item}>{item}</TableCell>
                                } else {
                                    return <TableCell classes={{ root: classes.tableCell }} align="right" key={item}>{txt.charts[item]}</TableCell>
                                }
                            }
                            )}
                        </TableRow>
                        ...
                        {stats.map(stat => {
                            return (
                                <TableRow key={stat.field} hover classes={{ root: classes.tableRow }}>
                                    <TableCell classes={{ root: classes.tableCellHead }} padding='none' component="th" scope="row">{stat.field}</TableCell>
                                    {statistics.includes('sum') && <TableCell classes={{ root: classes.tableCell }} align="right">{!stat.field.includes(' km2') ? isNumeric(stat.sum) ? separateThousands(formatNum(stat.sum, 0)) : '' : ''}</TableCell>}
                                    {statistics.includes('min') && <TableCell classes={{ root: classes.tableCell }} align="right">{isNumeric(stat.min) ? separateThousands(formatNum(stat.min, 0)) : ''}</TableCell>}
                                    {statistics.includes('max') && <TableCell classes={{ root: classes.tableCell }} align="right">{isNumeric(stat.max) ? separateThousands(formatNum(stat.max, 0)) : ''}</TableCell>}
                                    {statistics.includes('mean') && <TableCell classes={{ root: classes.tableCell }} align="right">{isNumeric(stat.mean) ? separateThousands(formatNum(stat.mean, 0)) : ''}</TableCell>}
                                    {statistics.includes('median') && <TableCell classes={{ root: classes.tableCell }} align="right">{isNumeric(stat.median) ? separateThousands(formatNum(stat.median, 0)) : ''}</TableCell>}
                                </TableRow>
                            )
                        })}
                    </TableBody>
                </Table>
                <Button style={{ maxWidth: 75, margin: '8px 8px 8px 0px' }} onClick={() => { this.props.toggleInfoDrawer() }} color="primary">{txt.button.close}</Button>
            </Paper>
        )
    };
        ...
        let rows = stableSort(featureInfo, getSorting(order, orderBy)).map((n, index) => {
            return (
                <TableRow hover key={index}>
                    <TableCell classes={{ root: classes.tableCellHead2 }} padding='none' component="th" scope="row">{statHeaders.map((header, i) => {
        ...
        featureInfo && featureInfo.map((feature, index) => {
            ...
            content.push(
                <div key={`InfoDrawer_${index}`} className={classes.infodiv}>
                    ...
                    {layers === [] || !layers.includes(feature.LAYERTITLE) && <Typography variant='overline'><strong>{feature.LAYERTITLE}</strong>{this.props.timeField && this.props.time && index === 0 && `(${this.props.timeField}: ${this.props.time})`}</Typography>}
        ...
        return (
            <LanguageContext.Consumer>{({ txt, lan }) => {
                return (
                    <div>
                        <Drawer
                            classes={{ paper: paperStyle || classes.paper, docked: paperStyle || classes.paper, paperAnchorDockedTop: classes.anchorTop, paperAnchorTop: classes.anchorTop }}
                            variant='persistent'
                            anchor='top'
