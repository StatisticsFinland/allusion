...
class AddressSearch extends Component {
  ...
  handleSubmit = event => {
    if(this.state.inputFieldValue && this.state.inputFieldValue !== ''){ 
      fetch(API + this.state.inputFieldValue + '.json?access_token=' + access_token + '&country=' + country + '&limit=' + limit)
        ...
        <List component="ul" dense={false} disablePadding={true}>
          {this.state.hits.map((element, index) =>
            <ListItem 
              ...
              aria-label="Search for address"             
              onClick={e => this.props.handleClick(element.center, 'address')}
            >
            <ListItemText primary={element.place_name} />
            </ListItem>
          )}
        </List>
      </div>
